...
const HEADERS = [
  ...
  'coffee', 'creatine', 'protein', 'carbs', 'calories', 'fat', 'fiber', 'sugar', 'sodium', 'work_periods', 'work_minutes', 'work_start', 'work_end', 'meditation_min', 'reading_min', 'journal_am', 'journal_pm',
...
function writeRow_(dateKey, d) {
  ...
  const vals = [
    dateKey,
    sum_(d.pushups), sum_(d.squats), sum_(d.pullups), sum_(d.dips), sum_(d.deadhang), sum_(d.kettlebell), sum_(d.sprints), sum_(d.jumprope),
    ...
    sum_(d.fat), sum_(d.fiber), sum_(d.sugar), sum_(d.sodium),
    ...
    (Array.isArray(d.restinghr) && d.restinghr.length) ? d.restinghr[d.restinghr.length - 1].v : '',
